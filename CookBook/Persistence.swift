...
struct PersistenceController {
    ...
    static let preview: PersistenceController = {
        ...
        // MARK: sample Ingredient
        var createdIngredients: [IngredientEntity] = []
        
        for i in 0..<3 {
            let ingredient = IngredientEntity(context: viewContext)
            ingredient.id = UUID()
            ingredient.name = "Sample Ingredient \(i)"
            ingredient.measurementUnit = "grams"
        ...
        recipe.id = UUID()
        recipe.name = "Halwa"
        recipe.instructions = "Instructions for Halwa"
        recipe.ingredients = NSSet(array: createdIngredients)
        do {
